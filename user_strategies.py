...
class SimpleMAStrategy(Strategy):
    ...
    def on_data(self, ts, data):
        for symbol in ["ADANIPORTS", "HAL", "ADANIGREEN", "TATAMOTORS"]:
            ...
            # More performant way to get closes. Index 3 is 'close'.
            closes = np.stack(bars[:])[:, 3].astype(float)
...
class OHLCReversalStrategy(Intraday):
    ...
    def __init__(self, broker, sizer=None):
        ...
        self._daily_agg = defaultdict(lambda: {"high": -np.inf, "low": np.inf, "open": None, "close": None})
    ...
    def _update_daily_data(self, ts, data):
        ...
        if self._current_day != ts.date():
            ...
            if self._current_day is not None:
                ...
                logger.debug(f"[{ts.date()}] New day. Previous day's OHLC finalized and stored.")
    ...
    def on_data(self, ts, data):
        ...
        if ts.time() == self.broker.market_open_time:
            for symbol in data:
                prev_day = self._prev_day_ohlc.get(symbol)
                if not prev_day or any(k not in prev_day for k in ["open", "high", "low", "close"]):
                ...
                if (
                    ...
                    and current_open < prev_day["low"] # Current open gapped down below previous day's low
                ):
                    logger.info(f"[{ts}] {symbol}: BUY condition met! Prev C({prev_day['close']:.2f}) < Prev O({prev_day['open']:.2f}) AND Curr O({current_open:.2f}) < Prev L({prev_day['low']:.2f})")
                    self.buy(symbol, entry_price=current_open, stop_loss_price=current_open * 0.98)
                elif (
                    prev_day["close"] > prev_day["open"] # Previous day was an up day
                    and current_open > prev_day["high"] # Current open gapped up above previous day's high
                ):
                    logger.info(f"[{ts}] {symbol}: SELL condition met! Prev C({prev_day['close']:.2f}) > Prev O({prev_day['open']:.2f}) AND Curr O({current_open:.2f}) > Prev H({prev_day['high']:.2f})")
                    self.sell(symbol, entry_price=current_open, stop_loss_price=current_open * 1.02)
                else:
                    logger.debug(f"[{ts}] {symbol}: No trade condition met. Prev Day OHLC: O={prev_day['open']:.2f}, H={prev_day['high']:.2f}, L={prev_day['low']:.2f}, C={prev_day['close']:.2f}. Current Open: {current_open:.2f}")
...
class OpeningRangeBreakoutStrategy(Intraday):
    ...
    def __init__(self, broker, sizer=None, opening_range_minutes=30, take_profit_factor=2.0):
        ...
        for symbol in data:
            ...
            if (
                ...
                if current_high > orb["high"]:  # Buy breakout
                    ...
                    if not self.buy(symbol, entry_price=entry_price, stop_loss_price=stop_loss_price):
                        continue
                    ...
                    if not self.sell(symbol, entry_price=entry_price, stop_loss_price=stop_loss_price):
                        continue
                    self._trade_taken[symbol] = True
                    self._position_details[symbol] = {
                        "side": "SELL",
        ...
        for symbol, bar in data.items():
            ...
            if position.size == 0:
                ...
                if c > vwap and l <= vwap:
                    ...
                    if self.buy(symbol, entry_price=entry_price, stop_loss_price=stop_loss_price):
                        self._position_details[symbol] = {
                            "sl": stop_loss_price,
                ...
                # Short entry: price is below VWAP and pulls back to touch it
                    ...
                    entry_price = c
                    stop_loss_price = entry_price * (1 + self.stop_loss_pct)
                    if self.sell(symbol, entry_price=entry_price, stop_loss_price=stop_loss_price):
                        self._position_details[symbol] = {
                            "sl": stop_loss_price,
                            "tp": entry_price * (1 - self.target_pct),
                        }
...
class DualMACrossoverStrategy(Strategy):
    ...
    def __init__(self, broker, sizer=None, symbol: str = "BSE", fast_period=50, slow_period=200):
    ...
    def on_data(self, ts, data):
        ...
        # Buy signal: Fast MA crosses above Slow MA
        if fast_ma[-1] > slow_ma[-1] and fast_ma[-2] <= slow_ma[-2] and position.size <= 0:
        ...
        # Sell signal: Fast MA crosses below Slow MA
        elif fast_ma[-1] < slow_ma[-1] and fast_ma[-2] >= slow_ma[-2] and position.size >= 0:
...
class RsiMeanReversionStrategy(Intraday):
    ...
    def _reset_daily_state(self):
        ...
        self._trade_taken.clear()
        self._position_details.clear()
        logger.info(f"New day {self._current_day}. Resetting RSI Mean Reversion daily state.")
    ...
    def on_data(self, ts, data):
        ...
        for symbol, details in list(self._position_details.items()):
            ...
            current_price = data[symbol][4]  # Using close price for check
            if details["side"] == "BUY" and (current_price >= details["tp"] or current_price <= details["sl"]):
                logger.info(f"Closing {symbol} BUY position due to SL/TP hit at {current_price:.2f}.")
                self.close(symbol)
                del self._position_details[symbol]
            elif details["side"] == "SELL" and (current_price <= details["tp"] or current_price >= details["sl"]):
                logger.info(f"Closing {symbol} SELL position due to SL/TP hit at {current_price:.2f}.")
                self.close(symbol)
                del self._position_details[symbol]

        # --- Main Strategy Logic ---
        for symbol in data:
            if self.bar_history[symbol].count < self.ema_period or self._trade_taken[symbol]:
                continue
            ...
            if current_close > ema and rsi < self.oversold_threshold:
                ...
                if self.buy(symbol, entry_price=current_close, stop_loss_price=sl):
                    self._trade_taken[symbol] = True
                    self._position_details[symbol] = {"side": "BUY", "sl": sl, "tp": current_close * (1 + self.take_profit_pct)}
                    logger.info(f"RSI_MR BUY for {symbol} at {current_close:.2f}. SL: {self._position_details[symbol]['sl']:.2f}, TP: {self._position_details[symbol]['tp']:.2f}")
            ...
            # Sell Signal: Trend is down, and price rallies to an overbought level
                ...
                if self.sell(symbol, entry_price=current_close, stop_loss_price=sl):
                    self._trade_taken[symbol] = True
                    self._position_details[symbol] = {"side": "SELL", "sl": sl, "tp": current_close * (1 - self.take_profit_pct)}
                    logger.info(f"RSI_MR SELL for {symbol} at {current_close:.2f}. SL: {self._position_details[symbol]['sl']:.2f}, TP: {self._position_details[symbol]['tp']:.2f}")
...
class BollingerMeanReversionStrategy(Intraday):
    ...
    """
    def __init__(self, broker, sizer=None, bb_period=20, bb_std=2, rsi_period=14, vol_period=20):
        super().__init__(broker, sizer=sizer, bar_history=max(bb_period, rsi_period, vol_period) + 5)
    ...
    def on_data(self, ts, data):
        ...
        for symbol, bar in data.items():
            ...
            if symbol in self.broker.positions and symbol in self._position_details:
                details = self._position_details[symbol]
                curr_price = bar[4]
                if (details['side'] == 'BUY' and (curr_price >= details['tp'] or curr_price <= details['sl'])) or \
                   (details['side'] == 'SELL' and (curr_price <= details['tp'] or curr_price >= details['sl'])):
                    self.close(symbol)
                    del self._position_details[symbol]
                continue
            ...
            if curr_close < lower[-1] and curr_rsi < 30 and vol_spike:
                ...
                if self.buy(symbol, entry_price=curr_close, stop_loss_price=sl):
                    ...
                    logger.info(f"BMR BUY: {symbol} at {curr_close:.2f} | RSI: {curr_rsi:.1f} | Target: {middle[-1]:.2f}")
                ...
                if self.sell(symbol, entry_price=curr_close, stop_loss_price=sl):
                    ...
                    logger.info(f"BMR SELL: {symbol} at {curr_close:.2f} | RSI: {curr_rsi:.1f} | Target: {middle[-1]:.2f}")
