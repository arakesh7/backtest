...
class Strategy:
    ...
    def __init__(self, broker, sizer=None, bar_history=100):
        ...
        self.bar_history = defaultdict(
            lambda: BarHistory(size=bar_history)
        )  # BarHistory(size=bar_history)
        ...
        self.market_close_time = (
            datetime.strptime(self.broker.market_close_time, "%H:%M:%S")
            - timedelta(minutes=5)
        ).time()
        self.market_open_time = datetime.strptime(
            self.broker.market_open_time, "%H:%M:%S"
        ).time()
    ...
    def close_on_eod(self, ts: pd.Timestamp):
        ...
        if self.market_close_time and ts.time() == self.market_close_time:
            logger.info(
                f"Market close time reached at {ts}. Closing all open positions."
            )
    ...
    def buy(
        ...
        if size is None:
            if entry_price is None:
                raise ValueError(
                    "`entry_price` must be provided to the buy method when `size` is not specified for sizer calculation."
                )
            size = self.sizer.get_size(
                self.broker, asset, entry_price, stop_loss_price, side="BUY"
            )
    ...
    def sell(
        ...
        if size is None:
            if entry_price is None:
                raise ValueError(
                    "`entry_price` must be provided to the sell method when `size` is not specified for sizer calculation."
                )
            size = self.sizer.get_size(
                self.broker, asset, entry_price, stop_loss_price, side="SELL"
            )
...
class Intraday(Strategy):
    def is_trades_taken_today(self, ts):
        ...
        return any(
            t.executed_at.date() == ts.date() for t in self.broker.trades.values()
        )
