...
class DefaultAnalyzer(Analyzer):
    ...
    def get_results(self):
        ...
        sharpe_ratio = (
            (equity_df["returns"].mean() / returns_std) * np.sqrt(trading_days_per_year)
            if returns_std != 0
            else 0
        )
        ...
        drawdown = np.where(
            cumulative_max > 0,
            (equity_df["equity"] - cumulative_max) / cumulative_max,
            0,
        )
        max_drawdown = np.min(drawdown) * 100

        closing_trades = [t for t in self.trades if t.pnl != 0]
        long_closes = [t for t in closing_trades if t.side == "SELL"]  # closed LONG
        short_closes = [t for t in closing_trades if t.side == "BUY"]  # closed SHORT

        num_close_trades = len(closing_trades)
        total_long = len(long_closes)
        total_short = len(short_closes)
        long_win_pct = (
            (len([t for t in long_closes if t.pnl > 0]) / total_long * 100)
            if total_long
            else 0.0
        )
        short_win_pct = (
            (len([t for t in short_closes if t.pnl > 0]) / total_short * 100)
            if total_short
            else 0.0
        )
        return {
            ...
            "number of close Trades": len(closing_trades),
            "Win Rate (%)": (len(winning_trades) / num_close_trades) * 100,  # win_rate,
            ...
            "Long Win (%)": round(long_win_pct, 2),
            "Short Win (%)": round(short_win_pct, 2),
            # ── Risk metrics ────────────────────────────────────────────────
